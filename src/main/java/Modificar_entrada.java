...
public class Modificar_entrada extends JPanel{
	
	private JList lista;
	private String[] Objetos={"  "};
	private JLabel label=new JLabel("   Numero de Entrada");
	...
	public Modificar_entrada(){
		...
		lista.setFixedCellWidth(600);
		lista.setFixedCellHeight(20);
		add(Arriba,BorderLayout.NORTH);
		Centro.add(new JLabel("   Entrada     Producto        Total          Fecha produccion             Fecha caducidad       Precio"),BorderLayout.NORTH);
...
//*******Eventos del boton BUSCA
			if(e.getSource()==Busca){
				if((No.getText()).equals(""))
					...
					cn=new ODBC();
					cn.Consulta("select * from entrada_producto where no_entrada="+No.getText());
					if(cn.Tx>0){
...
//********Eventos del boton MODIFICAR
			if(e.getSource()==Mod){
				...
				if(index>=0){
					...
					Agregar.getContentPane().add(new Titulo("Entrada de Producto","      Comercial Mexicana","Entrada: "+No.getText()),BorderLayout.NORTH);

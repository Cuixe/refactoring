...
public class Salida_producto extends JPanel{
	
	private String columnas[]={"Clave Producto","Cantidad","precio promedio"};
	private JTextField Firma[]=new JTextField[columnas.length];
...
//**********   Eventos del Boton AGREGAR			
			if(e.getActionCommand()=="Agregar"){
				...
				String SQL="Insert salida values("+No+",'"+fecha+"','"+ Firma[1].getText()+"')";
				if(!cn.Inserta(SQL))
					...
					Agregar.getContentPane().add(new Titulo("Salida de Producto","      Comercial Mexicana","Salida: "+No),BorderLayout.NORTH);
...
//**********   Eventos del Boton CANCELAR						
			if(e.getActionCommand()=="Cancelar"){
				ODBC cn=new ODBC();
				if(!cn.Inserta("delete from salida where id_salida="+No))
					JOptionPane.showMessageDialog(null,"La operacion genero el siguiente error\n"+cn.Salida);
				cambia(false);
			}
